...
function handleJoinGame(
    ...
    if (game.status !== 'waiting') {
        sendMessage(ws, 'error', { error: true, errorText: 'Game already started' });
        return;
    }


    if (game.players.some(p => p.index === userIndex)) {
        sendMessage(ws, 'error', { error: true, errorText: 'Already in this game' });
        return;
    }
    ...
    broadcast(users, game, 'update_players', game.players.map(p => ({
        name: p.name,
        index: p.index,
        score: p.score,
    })));
